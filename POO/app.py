        ...
        with open('usuarios') as archivo:
            ...
            while registro:
                if formulario.usuario.data == registro[0] and formulario.password.data == registro[1]:
                    ...
                    confirmacion = open("usuario_confirmado","w")
                    confirmacion.close()
                # Ingreso de usuario
...
@app.route('/carga_de_ventas')
def carga_de_ventas():
    if Validacion.ValidarArchivo('usuario_confirmado'):
        return render_template('carga_de_ventas.html')
...
def tabla():
    if Validacion.ValidarArchivo('usuario_confirmado'):
        if Validacion.ValidarArchivo('tabla.csv'):
            archivo_csv2 = Archivo.AbrirComoLista('tabla.csv')
            cliente = []
            codigo = []
            producto = []
            cantidad = []
            precio = []
            for lista in archivo_csv2:
                codigo.append(lista[0])
                producto.append(lista[1])
                cliente.append(lista[2])
                cantidad.append(lista[3])
    ...
    if Validacion.ValidarArchivo('usuario_confirmado'):
        if Validacion.ValidarArchivo('tabla.csv'):
            ...
            for lista in archivo_csv2:
                ...
                precio.append(lista[4])
            rango = len(codigo)
            return render_template('ultimas_ventas.html',nombre= 'Ultimas Ventas',nameParametro = 'TABLA', campo1 = codigo, campo2 = producto,campo3 = cliente,
                                campo4 = cantidad, campo5 = precio, rango= rango)
        else:
            return render_template('no_valido.html', mensaje= 'El csv archivo inexistente')
    else:
        return render_template('sin permiso.html')

@app.route('/agrega_venta', methods=['GET','POST'])
def agrega_venta():
    if request.method == 'POST':
        if Validacion.ValidarArchivo('usuario_confirmado'):
            nombre_cliente = request.form['nombre']
            ...
            nuevo_cantidad = request.form['cantidad']
            if Validacion.ValidarCampoCodigo2(codigo_producto) == False:
                return render_template('no_valido.html' , mensaje = 'El campo CODIGO debe estar compuesto por 3 letras máyusculas seguidas de 3 caracteres númericos')
            elif Validacion.ValidarCampoCantidad2(nuevo_cantidad) == False:
                return render_template('no_valido.html' , mensaje = 'El campo CANTIDAD solo permite números enteros')
            elif Validacion.ValidarCampoPrecio2(nuevo_precio) == False:
                return render_template('no_valido.html' , mensaje = 'El campo PRECIO solo premite números decimales')
            else:
                with open('tabla.csv','a', newline='') as archivo_csv:
                    escribir_csv = csv.writer(archivo_csv)
                    nueva_venta = [codigo_producto,nombre_producto,nombre_completo,nuevo_cantidad,nuevo_precio]
                    escribir_csv.writerow(nueva_venta)
